...
export default function PagoModal({ turno, onClose }) {
  ...
  const fetchPagos = useCallback(async () => {
    ...
    try {
      ...
      if (error.response) {
        ...
        alert(`Error al cargar pagos: ${error.response.data?.message || 'Error del servidor'}`);
      } else if (error.request) {
        console.error("No response received - backend might not be running");
        alert('No se pudieron cargar los pagos. El servidor backend no está disponible.');
      } else {
  ...
  const fetchDeudaStatus = useCallback(async () => {
    if (!turno?.paciente_dni) return;
    setLoadingDeuda(true);
    try {
      const res = await axios.get(`${API_BASE_URL}/api/pagos/paciente-deuda?paciente_dni=${turno.paciente_dni}`);
      if (res.data?.success) {
      ...
      if (error.response) {
        ...
        console.error("No response received - backend might not be running");
        // Set a default state indicating backend is not available
        setDeudaStatus({ error: 'Backend no disponible' });
      } else {
        console.error("Request setup error:", error.message);
      }
      // No mostrar alerta para evitar molestar al usuario si no puede cargar el estado de deuda
    }
